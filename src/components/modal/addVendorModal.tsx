...
const AddVendorModal: NextPage<IProps> = (props) => {
    // Add Company Modal
    const { isOpen, setIsOpen } = props
    const { handleSubmit, register, reset } = useForm<CreateVendorInput>()
    const { mutate, error } = trpc.useMutation(['vendor.create-vendor'], {
        onSuccess: () => {
            setIsOpen(false)
        },
    })
    const onSubmit = (values: CreateVendorInput) => {
        ...
        reset()
        values.email = values.email && values.email.trim().length > 0 ? values.email : undefined
        mutate({ ...values })
    }

    // Create data for job and company drop down list
        ...
        <Dialog className="flex mx-auto absolute z-50" open={isOpen} onClose={() => setIsOpen(false)}>
            ...
            <div className="fixed inset-0 flex items-center justify-center p-4">
                <Dialog.Panel className="flex flex-col bg-white px-10 py-5 items-center">
                    {/* title */}
                    <Dialog.Title className='text-2xl font-semibold mb-4'>Add Vendor</Dialog.Title>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className='mx-auto flex flex-col content-center justify-center items-center gap-2' >
                        ...
                        <div className='inputDiv'>
                            <input {...register('email')} className='inputField' type='email' placeholder='Email (optional)' />
                        </div>
